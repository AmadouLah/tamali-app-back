...
public class ProductController {
    ...
    public ResponseEntity<ProductDto> create(
        ...
        ProductDto dto = productService.create(
                ...
                request.purchasePrice(), request.productType(), request.unit(), request.taxable(), request.initialQuantity());
    ...
    public ResponseEntity<ProductDto> update(@PathVariable UUID id, @RequestBody ProductUpdateRequest request) {
        ProductDto dto = ResponseUtil.requireFound(productService.update(id,
                request.name(), request.reference(), request.categoryId(), request.unitPrice(), request.purchasePrice(),
                request.productType(), request.unit(), request.taxable()), "Produit", id);
        return ResponseEntity.ok(dto);
    }
